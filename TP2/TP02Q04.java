class Serie {
    ...
    public String extrairPais(String pais){
        ...
        for(i = 0; i < pais.length(); i++){
            if(pais.charAt(i) == '>' && i+1 < pais.length() && pais.charAt(i+1) != '<' && pais.charAt(i+1) != '&'){
                int j = i + 1;
                while(pais.charAt(j) != '<'){
                    resp += pais.charAt(j);
                    j++;
    ...
    public String extrairEmissora(String emissoraDeTelevisao){
        ...
        for(i = 0; i < emissoraDeTelevisao.length(); i++){
            if(emissoraDeTelevisao.charAt(i) == '>' && i+1 < emissoraDeTelevisao.length() && emissoraDeTelevisao.charAt(i+1) != '<' && emissoraDeTelevisao.charAt(i+1) != ' '){
                int j = i + 1;
    ...
    public String extrairTransmissao(String transmissaoOriginal){
        ...
        for(i = 0; i < transmissaoOriginal.length(); i++){
            if(transmissaoOriginal.charAt(i) == '>' && i+1 < transmissaoOriginal.length() && transmissaoOriginal.charAt(i+1) != '<'){
                int j = i + 1;
    ...
    public void imprimir() {
        MyIO.println(nome + " " + formato + " " + duracao + " " + paisDeOrigem + " " + idiomaOriginal + " " + emissoraDeTelevisao + " "
                + transmissaoOriginal + " " + numeroTemporadas + " " + numeroEpisodio);
    }
}
...
class TP02Q04 {
    ...
    public static boolean pesquisaBinaria(String palavra, Serie array[], int numEntrada){
        boolean resp = false;
        int dir = numEntrada, esq = 0, meio;
  
        while (esq <= dir){
           meio = (esq + dir) / 2;
           if(array[meio].getNome().equals(palavra)){
              resp = true;
              esq = dir + 1;
    ...
    public static void main(String[] args) throws Exception{
        ...
        s = MyIO.readLine();
        while (isFim(s) == false){
            if(pesquisaBinaria(s, entrada, numEntrada) == true){
                MyIO.println("SIM");
            }else{
                MyIO.println("NÃO");
            }
