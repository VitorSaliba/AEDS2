...
void ler_serie(Serie *serie, char *html)
{
    ...
    if(strcmp(serie->nome, "Grey's Anatomy") == 0){
...
void selecao()
{
    for (int i = 0; i < (n - 1); i++)
    {
        ...
        for (int j = (i + 1); j < n; j++)
        {
            if (global[menor].num_temporadas > global[j].num_temporadas)
            {
                menor = j;
            }
            if (global[menor].num_temporadas == global[j].num_temporadas)
            {
                if (strcmp(global[menor].nome, global[j].nome) > 0)
                {
                    menor = j;
                }
            }
        }
        swap(&global[menor], &global[i]);
    }
}
...
void insercaoPorCor(int n, int cor, int h){
    for (int i = (h + cor); i < n; i+=h) {
        Serie tmp = global[i];
        int j = i - h;
        while ((j >= 0) && (strcmp(global[j].idioma, tmp.idioma) > 0 || ((strcmp(global[j].idioma, tmp.idioma)) == 0 && (strcmp(global[j].nome, tmp.nome) > 0)))) {
            global[j + h] = global[j];
            j-=h;
        }
        global[j + h] = tmp;
...
int getMaior(Serie *global, int n) {
    Serie maior = global[0];

    for (int i = 0; i < n; i++) {
        if(global[i].num_temporadas > maior.num_temporadas ){
            maior = global[i];
        }
    }
    return maior.num_temporadas;
}

void countingsort(Serie *global, int n) {
    //Array para contar o numero de ocorrencias de cada elemento
    int tamCount = getMaior(global, n) + 1;
    int count[tamCount];
    Serie ordenado[n];
    ...
    for (int i = 0; i < n; count[global[i].num_temporadas]++, i++);

    //Agora, o count[i] contem o numero de elemento menores ou iguais a i
    for(int i = 1; i < tamCount; count[i] += count[i-1], i++);

    //Ordenando
    for(int i = n-1; i >= 0; ordenado[count[global[i].num_temporadas]-1] = global[i], count[global[i].num_temporadas]--, i--);

    //Copiando para o array original
    for(int i = 0; i < n; global[i] = ordenado[i], i++);
}
